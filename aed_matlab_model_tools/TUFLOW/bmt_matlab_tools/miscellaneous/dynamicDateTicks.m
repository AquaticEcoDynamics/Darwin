...
for i = 1:length(axH)
    datetick(axH(i), 'x','keeplimits'); % JN when datetick wants to blow out the limits
%     datetick(axH(i), 'x');
...
% z = zoom(figH(1));  % JN try to supress the above error
% p = pan(figH(1));
% d = datacursormode(figH(1));
...
% ------------ End of dynamicDateTicks-----------------------
    ...
    function updateDateLabel(obj, ev, varargin)
        ...
        %         set(axesInfo.Linked,'XTick', ticks);
        %
        %         % ONLY UPDATE LABELS IF THEY HAVE NOT BEEN SET TO EMPTY
        %         na = length(axesInfo);
        %         updatelabel = true(na,1);
        %         for aa = 1:na
        %             if isempty(get(axesInfo.Linked(aa),'XTickLabel'))
        %                 updatelabel(aa) = false;
        %             end
        %         end
        %
        %         set(axesInfo.Linked(updatelabel),'XTickLabel',labels)
